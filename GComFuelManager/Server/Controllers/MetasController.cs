                ...
                if (vendedor.Clientes is not null)
                {
                    ...
                    foreach (var meta in metas)
                    {
                        ...
                        foreach (var cliente in vendedor.Clientes)
                        {
                            ...
                            var result_cumplimiento = (double)(meta.Venta_Real ?? 1 / meta.Meta_Acumulada ?? 1);
                            meta.Cumplimiento_Mes = Math.Round(result_cumplimiento,2);

                            if (meta.Mes.Month != 1)
                            {
                                var meta_anterior = context.Metas_Vendedor.FirstOrDefault(x => x.VendedorId == vendedor.Id && x.Mes.Month == new DateTime(año_actual, meta.Mes.Month, 1).AddMonths(-1).Month
                                && x.Mes.Year == new DateTime(año_actual, meta.Mes.Month, 1).AddMonths(-1).Year);
                            ...
                            {
                                var resultado = (double)(meta.Resultado_Acumulado / meta.Meta_Acumulada);
                                meta.Porciento_Cumplimiento = Math.Round(resultado, 2);
                            }
                        }
                    }
                    return Ok(metas);
