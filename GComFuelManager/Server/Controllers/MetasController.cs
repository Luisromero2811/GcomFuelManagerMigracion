...
namespace GComFuelManager.Server.Controllers
{
    ...
    public class MetasController : ControllerBase
    {
        ...
        public MetasController(ApplicationDbContext context, UserManager<IdentityUsuario> userManager, VerifyUserId verifyUser, User_Terminal _Terminal)
        {
        ...
        public ActionResult Obtener_Metas([FromQuery] Metas_Vendedor metas_)
        {
            try
            {
                ...
                var vendedor = context.Vendedores.IgnoreAutoIncludes().FirstOrDefault(x => x.Id == metas_.VendedorId);
                if (vendedor is null)
                    return NotFound();

                List<int?> Clientes = context.Cliente.Where(x => x.Id_Vendedor == vendedor.Id && x.Activo && x.Terminales.Any(x=>x.Cod == id_terminal)).Include(x=>x.Terminales).Select(x => (int?)x.Cod).ToList();
                ...
                foreach (var meta in metas)
                {
                    ...
                    var ordenes_mes_actual = context.Orden.Where(x => x.Codest != 14 && x.Destino != null && x.Destino.Codcte != null && Clientes.Any(y => x.Destino.Codcte == y)
                                        && x.Fchcar != null && x.Fchcar.Value.Month == meta.Mes.Month && x.Fchcar.Value.Year == año_actual && x.Id_Tad == id_terminal)
                        .Select(x => new { x.Ref, x.Bolguiid, x.Obtener_Volumen }).ToList();

                    List<dynamic> ordenes_a_sumar = new();
                    ...
                    var suma_ordenes = ordenes_a_sumar.Sum(x => (double)x.Obtener_Volumen);
                    meta.Venta_Real += suma_ordenes;

                    if (meta.Mes.Month == 1)
                    {
                        var ordenes_mes_pasado = context.Orden.Where(x => x.Codest != 14 && x.Destino != null && x.Destino.Codcte != null && Clientes.Any(y => x.Destino.Codcte == y)
                        && x.Fchcar != null && x.Fchcar.Value.Month == new DateTime(año_actual, meta.Mes.Month, 1).AddMonths(-1).Month
                        && x.Fchcar.Value.Year == new DateTime(año_actual, meta.Mes.Month, 1).AddMonths(-1).Year && x.Id_Tad == id_terminal).ToList();
